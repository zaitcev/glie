...
def all_c(back):
    if back:
        for cx in xrange(len(tab_c), 0, -1):
            yield tab_c[cx-1]
    else:
        for cx in xrange(len(tab_c)):
            yield tab_c[cx]

def all_b(back):
    if back:
        for bx in xrange(len(tab_a), 0, -1):
            for c_str in all_c(bx % 2):
                yield tab_a[bx-1] + c_str
    else:
        for bx in xrange(len(tab_a)):
            for c_str in all_c(bx % 2):
                yield tab_a[bx] + c_str

def all_a(back):
    if back:
        for ax in xrange(len(tab_a), 0, -1):
            for b_str in all_b(ax % 2):
                yield tab_a[ax-1] + b_str
    else:
        for ax in xrange(len(tab_a)):
            for b_str in all_b(ax % 2):
                yield tab_a[ax] + b_str

def all_d():
    for dx in xrange(len(tab_d)):
